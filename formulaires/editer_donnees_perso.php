 ...
 **/
function formulaires_editer_donnees_perso_charger_dist(
	$id_auteur = 'new',
	$retour = '',
	$associer_objet = '',
 ...
 *     Hash du formulaire
 */
function formulaires_editer_donnees_perso_identifier_dist(
	$id_auteur = 'new',
	$retour = '',
	$associer_objet = '',
	$config_fonc = 'auteurs_edit_config',
...
function auteurs_edit_config($row) {
	...
	$Taut_changer_login = lire_config('moncompte/aut_changer_login', array());
	$config['edit_login'] = in_array(session_get('statut'),$Taut_changer_login) ? 1 : 0;
 ...
 *     Erreurs des saisies
 **/
function formulaires_editer_donnees_perso_verifier_dist(
	$id_auteur = 'new',
	$retour = '',
	$associer_objet = '',
	$config_fonc = 'auteurs_edit_config',
	...
	if (
		...
		$Taut_changer_login = lire_config('moncompte/aut_changer_login', array());
		if (!in_array(session_get('statut'),$Taut_changer_login)) {
 ...
 *   pour vérifier l'email saisi, avec un lien dans le mail sur l'action
 ...
 **/
function formulaires_editer_donnees_perso_traiter_dist(
	$id_auteur = 'new',
	$retour = '',
	$associer_objet = '',
	$config_fonc = 'auteurs_edit_config',
	...
	$nom     = _request('nom');
	$nom_ini = sql_getfetsel('nom', 'spip_auteurs', 'id_auteur=' . intval($id_auteur)) and
	...
	if ($associer_objet and $id_auteur = $res['id_auteur']) {
		...
		if ($objet and $id_objet and autoriser('modifier', $objet, $id_objet)) {
			include_spip('action/editer_donnees_perso');
	...
	if (
		$nom !== $nom_ini and
		lire_config('moncompte/modifier_nom_forum') === 'oui'
	) {
		sql_updateq('spip_forum', array('auteur' => $nom), 'id_auteur='.intval($id_auteur));
